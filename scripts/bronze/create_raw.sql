/* 
RAW LAYER

Purpose : In this layer, the data is directyly loaded to database without any transformation

Approaches :
    1. Raw data is stored using full load approach (tables are dropped and created)
    2. There is no transformation (e.g deduplication, null handling, rename columns)
    3. All the data types are mapped to VARCHAR
*/

CREATE SCHEMA IF NOT EXISTS bronze;
-- customers
DROP TABLE IF EXISTS bronze.customers;
CREATE TABLE bronze.customers (
    customer_id VARCHAR,
    customer_unique_id VARCHAR,
    customer_zip_code_prefix VARCHAR,
    customer_city VARCHAR,
    customer_state VARCHAR
);

-- order_items
DROP TABLE IF EXISTS bronze.order_items;
CREATE TABLE bronze.order_items (
    order_id VARCHAR,
    order_item_id VARCHAR,
    product_id VARCHAR,
    seller_id VARCHAR,
    shipping_limit_date VARCHAR,
    price VARCHAR,
    freight_value VARCHAR
);

-- Geolocation
DROP TABLE IF EXISTS bronze.geolocation;
CREATE TABLE bronze.geolocation (
    geolocation_zip_code_prefix VARCHAR,
    geolocation_lat VARCHAR,
    geolocation_lng VARCHAR,
    geolocation_city VARCHAR,
    geolocation_state VARCHAR
);

-- Order Payments
DROP TABLE IF EXISTS bronze.order_payments;
CREATE TABLE bronze.order_payments (
    order_id VARCHAR,
    payment_sequential VARCHAR,
    payment_type VARCHAR,
    payment_installments VARCHAR,
    payment_value VARCHAR
);

-- Orders
DROP TABLE IF EXISTS bronze.orders;
CREATE TABLE bronze.orders (
    order_id VARCHAR,
    customer_id VARCHAR,
    order_status VARCHAR,
    order_purchase_timestamp VARCHAR,
    order_approved_at VARCHAR,
    order_delivered_carrier_date VARCHAR,
    order_delivered_customer_date VARCHAR,
    order_estimated_delivery_date VARCHAR
);

-- Products
DROP TABLE IF EXISTS bronze.products;
CREATE TABLE bronze.products (
    product_id VARCHAR,
    product_category_name VARCHAR,
    product_name_lenght VARCHAR,
    product_description_lenght  VARCHAR,
    product_photos_qty VARCHAR,
    product_weight_g VARCHAR,
    product_length_cm VARCHAR,
    product_height_cm VARCHAR,
    product_width_cm VARCHAR
);

-- Sellers
DROP TABLE IF EXISTS bronze.sellers;
CREATE TABLE bronze.sellers (
    seller_id VARCHAR,
    seller_zip_code_prefix VARCHAR,
    seller_city VARCHAR,
    seller_state VARCHAR
);

-- Order Reviews 
DROP TABLE IF EXISTS bronze.order_reviews;
CREATE TABLE bronze.order_reviews (
    review_id VARCHAR,
    order_id VARCHAR,
    review_score VARCHAR,
    review_comment_title VARCHAR,
    review_comment_message VARCHAR,
    review_creation_date VARCHAR,
    review_answer_timestamp VARCHAR
);

-- Product Category Name Translation
DROP TABLE IF EXISTS bronze.category_translation;
CREATE TABLE bronze.category_translation (
    product_category_name VARCHAR,
    product_category_name_english VARCHAR
);
